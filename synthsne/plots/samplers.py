...
def plot_obse_samplers(lcset_name, lcset_info, obse_sampler_bdict,
	...
	for kb,b in enumerate(band_names):
		...
		if original_space:
			label = '$p(\\mu_{ij},\sigma_{\\mu ij})$'+f' {lcset_name} samples'
			...
			if add_samples:
				...
				ax.plot(new_obse, new_obs, 'r.', markersize=2, alpha=1); ax.plot(np.nan, np.nan, 'r.', label='$\hat{p}(\\mu_{ij},\sigma_{\\mu ij})$ samples (KDE)')
			...
			label='$p(\\mu_{ij}'+"'"+',\sigma_{\\mu ij}'+"'"+')$'+f' empirical samples'
			...
			for p_idx in range(len(obse_sampler.distrs)):
				...
				if p_idx%10==0:
					...
					c = colors[p_idx]
					label = '$q(\sigma_{\\mu ij}'+"'"+'|\\mu_{ij}'+"'"+')$ Normal distribution fit'
		...
		title = ''
		title += f'{latex_bf_alphabet_count(kb)} Joint distribution; band={b}'+'\n'
	...
	suptitle = ''
	# suptitle += f''+'\n'
	suptitle += f'Gaussian distribution fits for the conditional observation-error distribution'+'\n'
